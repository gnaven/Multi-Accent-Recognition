...
class AccentModel(torch.nn.Module):
    ...
    def __init__(self, blockSize,input_dim, hidden_dim, batch_size, output_dim=17, num_layers=2):
...
def train(nepochs,trainDataloader,valDataloader,learningRate):
    ...
    blockSize = 201
    labelSize = 15
    model = AccentModel(blockSize=201,input_dim = 201, hidden_dim=100, batch_size=2, output_dim=17, num_layers=2)
    model.to(device) 
        ...
        with trange(len(trainDataloader)) as t:
            ...
            for index in t:
                try:
                    ...
                    t.set_description(f"epoc : {epoch}, loss {totalLoss/(index+1)}")
                    if index>1000:
                        break
...
if __name__ == "__main__":
    ...
    trainDataloader = DataLoader(trainDataset,batch_size=15,shuffle=True, num_workers=0,collate_fn=Data.collate_fn)    
    
    valDataset = Data.VoiceData(Path_Wav,Path_Meta+'dev.tsv')
    valDataloader = DataLoader(valDataset,batch_size=15,shuffle=True, num_workers=0,collate_fn=Data.collate_fn) 
